panelNNET.est <- function(y, X, hidden_units, fe_var, maxit, lam, time_var, param, parapen, parlist
  ...
  lossfun <- function(pl, skel, lam, parapen){
    ...
    mse <- mean((y-yhat)^2)
    plist <- relist(pl, skel)
    loss <- mse + lam*sum(c(plist$beta_param*parapen, 0*plist$beta_treatment, plist$beta, unlist(plist[!grepl('beta', names(plist))]))^2)
    return(loss)
  }
  ...
  calc_hlayers <- function(parlist, normalize = FALSE){
    ...
    for (i in 1:nlayers){ 
      if (i == 1){D <- X} else {D <- hlayers[[i-1]]} #bottom layer d is x #D = Data
      if (bias_hlayers == TRUE){D <- cbind(1, D)} #hlay[[length(hlay)]] %*% T_1
      if (normalize == TRUE){
  ...
  #add an arg here
  calc_grads <- function(plist, hlay = NULL, yhat = NULL, curBat = NULL, droplist = NULL, dropinp = NULL, treatment = NULL){
    ...
    if (!is.null(treatment)){
      grads[[length(grads)]] <- (grads[[length(grads)]] * (treatment))
    } else {
      grads[[length(grads)]] <- (grads[[length(grads)]])
    }
  ...
  if (is.null(parlist)){#random starting weights
    ...
    #Add the treatment effect with the derived variables
    if (!is.null(treatment)){
      warning('WARNING: panelNNET for heterogeneous treatment effects is still highly experimental.  
              the gradient descent algorithm appears to suffer badly from local minima 
              and the standard errors of the treatment effects appear to be extremely buggy.')
